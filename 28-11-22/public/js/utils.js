...
const param2El = formEl.param2; //input primo numero
const btnEl = document.querySelectorAll(".btn"); //input operazioni
const resEl = document.querySelector("#risultato"); //risultato

const GET = async (url) => {
  await fetch(url)
    .then((res) => res.json())
    .then((data) => resEl.textContent = data.risultato);
}
